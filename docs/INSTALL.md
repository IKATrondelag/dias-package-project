...
### tkinter
Applikasjonen bruker `tkinter` for GUI. tkinter er ikke alltid inkludert som standard i Python-installasjoner:

| Plattform | Kommando |
|-----------|----------|
| Ubuntu/Debian | `sudo apt-get install python3-tk` |
| Fedora/RHEL | `sudo dnf install python3-tkinter` |
| macOS (Homebrew) | `brew install python-tk@3.12` (tilpass versjonsnummer) |
| Windows (python.org) | Re-kjør Python-installeren → "Modify" → huk av "tcl/tk and IDLE" |
| conda | `conda install tk` |

Verifiser at tkinter er tilgjengelig:
```bash
python -c "import tkinter; print('tkinter OK')"
```

> **Merk ved bygging med PyInstaller:** tkinter må være tilgjengelig i det Python-miljøet som brukes til bygget. Uten tkinter vil PyInstaller-bygget feile. Se `README.md` → *Build Troubleshooting* for detaljer.
