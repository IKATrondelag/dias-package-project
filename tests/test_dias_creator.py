...
class TestMetadataHandler(unittest.TestCase):
    ...
    def test_save_metadata_to_xml_and_reload(self):
        """Saving GUI metadata to XML should create a readable submission METS file."""
        gui_metadata = {
            'package_type': 'SIP',
            'label': 'Saved Metadata Test',
            'record_status': 'NEW',
            'archivist_organization': 'Test Archive',
            'system_name': 'Test System',
            'creator_organization': 'Test Creator',
            'submission_agreement': 'AGR-001',
            'descriptive_title': 'Test Package Title',
            'descriptive_creator': 'Test Metadata Creator',
            'descriptive_identifier': 'meta-001'
        }

        metadata_path = os.path.join(self.temp_dir, 'saved_metadata.xml')
        self.handler.save_metadata_to_xml(metadata_path, gui_metadata)

        self.assertTrue(os.path.exists(metadata_path))

        reloaded = self.handler.load_metadata_from_xml(metadata_path)
        self.assertIsNotNone(reloaded)
        self.assertEqual(reloaded.get('package_type'), 'SIP')
        self.assertIn('descriptive_metadata', reloaded)
        self.assertEqual(reloaded['descriptive_metadata'].get('title'), 'Saved Metadata Test')
        self.assertTrue(reloaded.get('agents'))
